...
import pandas as pd
global Record
from  MondeVert.SHAINE_MonderVert.SHAINE_WIZARD_PROMPTS import Wedding_Prompts as WeddingP
from MondeVert.SHAINE_MonderVert.SHAINE_WIZARD_PROMPTS import Social_Media_SHAINE as sms
from secrets import randbelow
from MondeVert.SHAINE_MonderVert.SHAINE_WIZARD_PROMPTS import Long_User_Prompts as lup, User_Prefs as up, \
...
#import urllib2
import webbrowser
from MondeVert.SHAINE_MonderVert.Utilities import Common_Utilities as cu, ShakesBot as s
import platform
import subprocess
import requests
import random
...
class MondeVert():
    ...
    def makeArt(self, Prompt='', SavePath = up.AI_Art_Path):
        ...
        fname = f"{''.join([c for c in FileName.strip().replace(' ', '_') if c.isalnum() or c == '_'])}.jpg"



        if os.path.isfile(os.path.join(up.AI_Art_Path, '\'', fname)):
            fname = fname.split(".")[0] + f".{''.join(random.choice(string.ascii_letters) for x in range(5))}.jpg"
    ...
    #Create new subfolder etc for this filepath
        ...
        SavePath2 = SavePath + '\\' + folder
        Title2 = SavePath2 + Title1 + self.current_time

        cu.Check_Folder_Exists(SavePath2)
        ...
        try:
            ...
            # print(Title2)
            filename = Title2
            cu.SaveCSV(data, filename, SavePath2)
    ...
    #I can likely use this for all of the ChatGPT conversations
        ...
        FilePathnew = FilePath[:-4] + '_' + Voice_fix + '_' + self.current_time + '.mp3'
        FilePathnew2 = FilePath[:-4] + '_' + Voice_fix + '_' + self.current_time + '_Italian.mp3'
        ...
        FilePathnew= FilePath[:-4] + '_'+Voice_fix + '_'+ self.current_time +  '.mp3'
        NewTempPath = cu.SaveText2Audio(Text, SavePath=SavePath,  FilePath=FilePathnew, Voice = Voice, FileName=FileName)
        print(NewTempPath)
        try:
            shutil.move(NewTempPath,FilePathnew)
            print('Audio File Saved in the following location: ' + FilePathnew)
            ...
            print(NewTempPath)
    def Basic_GPT_Query(self,   Line2_Role  , Line3_Format,Line4_Task,Special = '',Line1_System_Rule = up.system_TextJoaT, crazy = .5, Subject= '', Outline = '', Allowed_Fails = 8, SaveFile = False,MakeArt = False, Mode = 'SHAINE SAYS', SavePath= self.SavePath):#use this to create art style for the work
        ...
        1). """ + Line1_System_Rule + Line2_Role+"""
        2).""" + Line2_Role+ """
        3).""" + Line3_Format+ """
        4).""" + Line4_Task
        ...
        while KeepGoing == False and KillSwitch < Allowed_Fails:
            ...
            if MakeArt ==True:
                ...
                PicNewPath = SavePath + '\\' + Mode + '\\' + Title + '.jpg'
...
#Make sure to start a master tracker with this information, use the writer persona as part of the data stored (TimeStamp, WriterPersona, Outline, Format, and/or put the prompt together for reference
    ...
    def Translate_english2Italian(self,Line1 = up.Translate_Sys, Language = 'Italian', Text = 'Sorry no text was sent but translate this instead, Monde Vert apologizes', Line2 = up.Translate_PictureBook_Line2, Line3= up.Translate_PictureBook_Line3,Line4= up.Translate_PictureBook_Line4, SavePath = up.Children_audio_book, File_Name = 'Italian Translation'):



        Line3 = Line3 + Text + "] to [" +Language + "] " + Line4
        Line4 = 'Have fun!'
...
#********************************************************************************************************************************
        ...
        try:
            for i in range (1,Episode_Count+1):
                ...
                cu.SaveText2Audio(SavePath=SavePath, FileName=FileName_Audio, Voice=Voice, Neural='Neural', Mode='AUDIOBOOK')
        except:
...
#This is just the characters, need to do this for scenes etc too
    ...
    def MondeVertMenu_up(self, Mode='Basic', Role = '', System = ''):
        ...
        if Mode == 'ScreenPlay':
            ...
            MondeVert.Basic_GPT_Query(self, Line1_System_Rule= sms.Clean_Post_Line1, Line2_Role=sms.Clean_Post_Line2, Line3_Format=sms.Clean_Post_Line3, Line4_Task=sms.Clean_Post_Line4,Mode='Social_Media_Clean_Post' )
           ...
           MondeVert.MondeVert_Audio_Video_Story(self)
            ...
            MondeVert.MondeVert_SHAINE_WeddingVows(self,  Mode='Wedding Vows')
...
#This was misguided lol but I think getting rid of prompt for now is ok eventually maybe this will cause an issue, going to be fully reworked soon
            ...
            try:
                ...
                filename = Title1
                CSVLocation = cu.SaveCSV(data, filename, SavePath2)
                title1 = cu.CleanFileName(Title)
                try:
            ...
            try:
                cu.send_email_w_attachment_gmail(up.to, up.subject, Project_Description, FilePath_email)
                print("Gmail email sent")
                cu.send_email_w_attachment_outlook(up.to, up.subject, Project_Description,  FilePath_email)
                print("Outlook email sent")
            except:
                print('email not send, its possible file was not created')
    ...
    def GPTArt2(self, crazy='', prompt=sms.ArtPrompt_Clean_Social_Media_Post_Line2_Prompt,
    ...
    def Make_a_ScreenPlay(self, System = '',Mode='ScreenPlay'):
        ...
        for x in range(0, 4):
            ...
            try:
                ...
                filename = cu.SaveCSV(data[x], filename1[x], SavePath2)
                # MondeVert.SaveText(self,df1,'MondeVert Assistant', 'Full Transcript')
        ...
        ArtistPoetInfo = 'Lyrics Written By: ' + up.Song_Writer + '      (' + 'Artwork by: ' + up.AI_ArtistName + ')'
        cu.NamePoemSavePoem(self, prompt, ArtPaths, Prompts_Used, ArtistPoetInfo, title=Title,
                                       SavePath=SavePath2, Mode='Screen Play' + Mode)
